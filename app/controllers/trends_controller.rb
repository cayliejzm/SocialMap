...
before_action :authorise_twitter
  ...
  def index
    ...
    if time_gap
      trends = trends.where(created_at: ((Time.now + (time_gap.to_i - .5).hours)...(Time.now + time_gap.to_i.hours )))
    end
...
def map
  ...
  maptrends = maptrends.where(created_at: ((Time.now + (time_gap.to_i - .5).hours)...(Time.now + time_gap.to_i.hours )))
...
private
  ...
  def destroy_old_tweets
    Trend.where("created_at < ?", Time.now - 0.5 days).destroy_all
  end
